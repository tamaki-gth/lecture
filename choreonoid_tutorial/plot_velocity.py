...
# CSVファイルを読み込む
df = pd.read_csv("inference_log_0_34.csv")
...
# 目標速度
plt.plot(df["time"], df["cmd_vx"], label="Target velocity", linewidth=2)
# 前進速度
plt.plot(df["time"], df["base_vx"], label="Base velocity", linewidth=2)

# 軸ラベル
plt.xlabel("Time [s]")
plt.ylabel("Velocity [m/s]")
# タイトル
plt.title("Target Velocity vs Base Velocity")
# 凡例
plt.legend()
# グリッド
plt.grid(True)
...
plt.plot(df_cycle["time"], df_cycle["cmd_vx"], label="Target velocity", linewidth=2)
plt.plot(df_cycle["time"], df_cycle["base_vx"], label="Base velocity", linewidth=2)
...
plt.title(f"Velocity during 1 Stride ({start_time}s - {end_time}s)")
...
plt.tight_layout()
# 2枚目の画像を保存
plt.savefig("velocity_cycle.png", dpi=300)
print("Saved as velocity_cycle.png")
